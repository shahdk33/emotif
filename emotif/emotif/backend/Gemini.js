...
const{connectFirebase, addAievents} = require("./firebase.js");
async function askGemini(events, emotions) {
    try {
        ...
        const aievents = JSON.parse(result.response.text());
        aievents.forEach((event)=>{addAievents(db,event); console.log(event);});
