...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    const { aievents, events, emotions } = req.body;  // Receive events and emotions in the body
    ...
    try {
      ...
      const prompt = `Given the calendar events of a user:${JSON.stringify(events)} and their logged emotions for a day:${JSON.stringify(emotions)} and previously suggested events ${JSON.stringify(aievents)}, could you create ONE suggestion for an activity that the user could do in their free time that would make the user feel better? Specifically say what activity and the reason for suggesting that activity. Please provide your answer in the following format without the json keyword or the three backticks at the begining or end, JUST plain text as this format: [{"date": "", "endtime": "", "name": "AI: ", "starttime": ""}]. for example: [{"date":"03-03-2025","endtime":"23:00","name":"AI: Relaxing bath or shower","starttime":"22:00"}]. Please do not include any other text, JUST the json format of the recommended activities.`;
